...
For \textbf{Linux}, this might look like this, if you have the IP address suggested in section 2.1.1 and your username is ``bob'':

\begin{verbatim}
 scp -rp data bob@178.122.1.50:/home/bob
\end{verbatim}

For Windows, it would look something like this:

\begin{verbatim}
 scp -rp data bob@178.122.1.50:C:\users\bob\Desktop
\end{verbatim}

You can now turn the Pi off with:
\begin{verbatim}
 sudo halt
\end{verbatim}

If the system is still logging, disconnect the Ethernet cable, and replace the red jumper wire. 

\subsubsection{Checking the data}

Once you are logged in, you can check that the Pi is logging properly. Type:
\begin{verbatim}
 cd data
\end{verbatim}

To enter the \textit{data} directory. Now type:
\begin{verbatim}
 cat data.csv
\end{verbatim}

And you will see the contents of the CSV file - if it's very long, you will just see the last few pages. \newline
