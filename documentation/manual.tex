...
\begin{itemize}
 ...
 \item 
 \item The default IP address is 178.122.1.200. The password is \textit{gogerddan}
 \item Type \begin{verbatim}
             sudo raspi-config
            \end{verbatim}
 \item Select the second option in the dialog box, labelled ``Expand root partition to fill SD card''
\end{itemize}
...
\url{https://www.raspberrypi.org/documentation/usage/camera/} (just the physical ``Connecting the Camera'' part - the rest is already set up on the SD card). 
...
\begin{itemize}
 ...
 \item The SD card will be mounted like a normal USB stick or external drive. It will probably appear as two drives - pick the largest one
 \item Find the directory called \textit{/home/pi/data}
 \item Copy the directory to your computer
\end{itemize}

The second method is similar to the process in section 2.1.1. You will need to have the connection already set up so that you can SSH into the Pi. If you are on Linux, you can copy the right directory using SCP:

\begin{verbatim}
  scp -rp /home/pi/data user@your-computer:/home/user/destination
\end{verbatim}

For Windows 7, do the following:

\begin{itemize}
 \item Download WinSCP from \url{https://winscp.net/eng/download.php}
 \item
 \item
 \item
\end{itemize}
